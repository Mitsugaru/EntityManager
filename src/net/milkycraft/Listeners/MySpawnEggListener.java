...
/**
 * The listener interface for receiving mySpawnEgg events. The class that is
 * interested in processing a mySpawnEgg event implements this interface, and
						...
						if (!onDecidingSpawningFactors(seg)) {
							...
							if (!player
								...
								if (seg.getEntityBreed().getCategory() == EntityCategory.ANIMAL) {
									Spawnegg.econ.withdrawPlayer(playa, animal);
									...
									Spawnegg.econ.withdrawPlayer(playa, npc);
